...
void Application::parseArguments(const QStringList &arguments, Options *options)
{
    ...
    if (positionalArguments.count() == 2) {
        ...
        if (argument.endsWith(".qml")) {
            ...
            if (document.isNull()) {
                qWarning() << "Document path is outside the workspace directory: " << argument;
                parser.showHelp(-1);
            } else if (!document.existsIn(options->workspace())) {
                qWarning() << "Document does not exist: " << document.absoluteFilePathIn(options->workspace());
                parser.showHelp(-1);
            } else if (!document.isFileIn(options->workspace())) {
                qWarning() << "Document is not a regular file: " << document.absoluteFilePathIn(options->workspace());
                parser.showHelp(-1);
            }
